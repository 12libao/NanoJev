...
以及 ViZDoom Basic / Predict Position。当前采集包含 **228 个 Jev 引导回合、16,637 次执行决策**，
其中 14,503 次由模型选择，2,134 次为单候选强制动作。策略 SFT 先进行 200 次基础更新，
再在扩展任务集上进行 100 次更新。

随后冻结策略，以 15% 均匀探索采集完整回合，用实际终局结果标注**每个已执行的状态与动作**。
概率训练比较 `paired_brier_pg`（32 次预测抽样）、直接 Brier 和 CE，均保留 25% 的策略监督权重；
Q 控制器选择动作后，重新采集下一轮数据。

**Choice** 输出候选动作上的一个分布；**Boolean Q** 输出各动作独立的成功概率，
它们不需要加起来等于一。后续策略改变时，下一轮采集也会更新对应的概率目标。
详见[实现与训练命令](docs/UNIFIED_GAMES.md)。
