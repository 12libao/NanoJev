// Fresh Jev measurements for closed-loop comparisons. Credentials stay local.
...
if(unresolved.size)throw Error('Unresolved prior API request; reconcile credits before retrying');
...
async function evaluate(publicState){
  ...
  if(spent+reserved+reservation>budget)throw Error('Application API budget reached');
  ...
  try{
    ...
    throw Error('Jev measurement failed; no retry or fallback');
...
for await(const line of readline.createInterface({input:process.stdin,crlfDelay:Infinity})){
  ...
  try{
    ...
    await Promise.all(Array.from({length:Math.min(concurrency,results.length)},async()=>{
      while(next<results.length&&!failed){const index=next++;try{results[index]=await evaluate(request.states[index]);}catch(e){failed=e;}}
    }));
